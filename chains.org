...
#+begin_src lisp
  (define-task-input input-data)
  (define-task-input distance-away-from-ground-truth)
...
given task and chain you use the ~DEFINE-TASK-INPUT-FUNCTION~ macro.
#+begin_src lisp
  (define-task-input-function input-data geometric-cross-validation ((occlusion synthetic-occlusion))
    (task-value occlusion))
  
  (define-task-input-function distance-away-from-ground-truth geometric-cross-validation ((object distance-away-from-ground-truth))
    (distance object))
#+end_src
...
The ~DEFINE-TASK-INPUT~ and ~DEFINE-TASK-INPUT-FUNCTION~ provide a
flexible method of computing input values to a task conditioned on the
tasks in the chain.
...
set of experiments which are to be performed.

Creating the experiments is a two step process. The first step is
...
only contains the information needed to generate the tree, it doesn't
contain the experiments themselves.

A design document contains information needed to generate the set of
experiments. Creating a design document in the ~CHAINS~ system is
achieved with the ~DEFINE-DESIGN~ macro.
#+begin_src lisp
  (defmacro define-design (name design-options &body levels))
#+end_src
The argument ~NAME~ is a symbol naming the design that is being
defined. ~DEFINE-OPTIONS~ is a list of design options. These options
will be specified later. The argument ~LEVELS~ contains information
about the hierarchy of the tree. A diagram illustrating the role of
~LEVELS~ in the ~DEFINE-DESIGN~ is as follows.
...
document outlining the set of experiments that to be generated for the
congealing problem.
#+begin_src lisp
...
The first two lines of the ~DEFINE-DESIGN~ are self explanatory.

Lets consider what the first expression means
#+begin_src lisp
  (image-appearance-and-lighting (:lighting-variation 0 0.1 0.2 0.3 0.4)
                                 (:number-of-subjects 1 2 5 10 20)
                                 (:samples-per-subject 1 5 10))
...
tree. The tasks produced with the next expression in the
~DESIGN-DOCUMENT~ will be children to the 
#+begin_src lisp
(image-cross-validation (:sample (:splice (loop :for x :from 0 :below 10 :collect x))))
#+end_src
