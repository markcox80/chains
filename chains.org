...
#+begin_src lisp
  (defmacro easy-block-name (block-name))
#+end_src
...
- ~symbol~ :: This generally refers to the base class of a
              block. e.g. All congealing algorithm blocks inherit from
...
- ~(order base &rest names)~ :: Order blocks that inherit from the
     base class ~BASE~ in the specified order of sub-classes. It is an
     error if a block is found with that inherits from ~base~ but no
     name is present.
...
Once the set of experiments are documented, generating them is
easy. The following is an example document outlining the set of
experiments that to be generated for the congealing problem.
#+begin_src lisp
  (define-design congealing
    ...
    ((image-cross-validation (:sample (:eval (loop :for x :from 0 :below 10)))))
    ...
    ((geometric-cross-validation (:sample (:eval (loop :for x :from 0 :below 3)))))
