...
must be prepared. The function ~PREPARE~ performs this task.
#+begin_src lisp
  (defun prepare (directory &key if-exists))
#+end_src
The argument ~IF-EXISTS~ determines what to do if the given
~DIRECTORY~ is not empty.

A task's operation can not be executed directly as it is conditioned
on tasks in the chain. A chain can be executed using the function
~PERFORM~.
#+begin_src lisp
  (defun perform (area chain &key force &allow-other-keys))
#+end_src
The ~DIRECTORY~ argument specifies where the results of the execution
are to be stored. The ~CHAIN~ argument is the chain to be
executed. The keyword argument ~FORCE~, specifies that all of the
tasks should be executed, regardless of whether or not they have been
executed.

An operation can obtain the value of the ~FORCE~ argument via the
~*FORCED*~ dynamic variable. All keyword arguments given to ~PERFORM~
can be obtained using the function ~PERFORM-PROPERTIES~.
#+begin_src lisp
(defvar *forced*)
(defun perform-properties ())
#+end_src
