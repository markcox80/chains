...
#+begin_src sh
  $ ./program --help
  Usage: [options] [custom options] <tree> <depth> <leaf number>
  ...
  <tree> The tree containing the chains that are to be executed.
  <depth> The depth of the tasks that are to be executed.
  <leaf number> The index of the leaf at <depth> that is to be executed.
#+end_src

The ~DEFINE-PROGRAM~ macro provides a very thin wrapper over the
...
#+begin_src lisp
  (defun prepare-oge-script (directory tree pathname &key (if-exists :error))
#+end_src
where ~DIRECTORY~ is the location to store the generated files, ~TREE~
is the tree to be executed, ~PATHNAME~ is the pathname to the
generated program. 
...
- ~program-arguments.sh~ :: A user editable file for passing command
     line arguments to the program.
- ~oge-arguments.sh~ :: A user editable file for passing command line
     arguments to the grid engine scheduler.
- ~tree.sexp~ :: The file containing the tree to be executed by the
                 grid engine.
...
- ~:OVERWRITE~ :: Overwrite ~oge.sh~, ~oge-program.sh~ and
                  ~tree.sexp~. Do not touch ~program-arguments.sh~ and
                  ~oge-arguments.sh~.
- ~:OVERWRITE-ALL~ :: Overwrite all of the above files.
