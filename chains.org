...
#+begin_src lisp
  (defun prepare-oge-script (directory area tree pathname &key (if-exists :error)))
#+end_src
where ~DIRECTORY~ is the location to store the generated files, ~AREA~
is a prepared area, ~TREE~ is the tree to be executed, ~PATHNAME~ is
the pathname to the generated program.
