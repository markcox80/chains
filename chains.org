...
#+begin_src ditaa :file input-data.png
  ...
  +------+------------------------+
         ...
         v
  +------------------------+
  | Image Cross Validation |<--=-Images determined here
  +------+-----------------+
         |
         v
  +---------------------+
...
synthesising a set of random samples with the same
statistics. Meaning, when the image cross validation task executes, an
entirely different set of images with same level of appearance
variation and lighting conditions are selected. Similarly, the
geometric cross validation is a new set initial guesses on where the
object is for the same set of images.
