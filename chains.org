...
The last expression ~(= symbol name)~ utilises arity two predicate
specified via the ~:TEST=~ slot definition argument permitted by
~DEFINE-BLOCK~. For example, in the definition of the
~GEOMETRIC-CROSS-VALIDATION~ block one would specify
#+begin_src lisp
  (define-block geometric-cross-validation ()
    ((sample
      :initarg sample
...
groups and the chains within the groups according to a sorting
expression. A sorting expression is of the form

- ~(>= symbol name)~ :: Like the ~=~ expression previously, but using
     the ~:TEST>=~ slot definition argument.
- ~(<= symbol name)~ :: Similar to the ~>=~.
- ~(block base name-1 name-2 ..)~ :: Order blocks with base class
     ~BASE~ in the specified order of sub-classes.
