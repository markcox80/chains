...
Each node of the experiment tree has its own directory in order to
store the blocks own data and the data used by the chains system. The
name of this directory is computed using the function ~BLOCK-NAME~.
#+begin_src lisp
(defgeneric block-name (block))
#+end_src

The macro ~DEFINE-BLOCK~ provides a convenient method of implementing
this function via the ~:NAME~ form.
#+begin_src lisp
    (define-block geometric-cross-validation ()
      ((sample
        :initarg sample
        ...
        :documentation "The index of the random sample."))
      (:name (object)
        (format nil "gcv-~3,'0d" (sample object))))
#+end_src
