...
** Interacting with the execution
The above execution model works well for most tasks. There is a class
of tasks which unfortunately do not fit this model neatly and require
special mention. The class of tasks being referred to are tasks that
execute an iterative algorithm whose output is continually reprocessed
until some sort of criteria is met e.g. non-linear optimisation.
