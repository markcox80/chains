...
- ~symbol~ :: Find two tasks, one from each chain, whose classes are
              ~EQUAL~ and of type ~SYMBOL~. e.g. all congealing
              algorithm tasks inherit from ~ALGORITHM~, thus
              specifying ~(quote ALGORITHM)~ would group all chains
              that use the same algorithm.
- ~(= symbol :static)~ :: Find two tasks, one from each chain, which
     are of type ~SYMBOL~ and are equal according to the predicate for
     the task class ~SYMBOL~.
- ~(= symbol :dynamic)~ :: Find two tasks, one from each chain, whose
     classes are ~EQUAL~, are of type ~SYMBOL~, and are equal
     according to the task predicate for the found task class.
- ~(= symbol)~ :: Find two tasks, one from each chain, which are of
                  type ~SYMBOL~ and are equal according to the
                  predicate for the task class ~SYMBOL~. An error is
...
- ~(and &rest expressions)~ :: This expression specifies that all the
     chains in a single group satisfy all ~EXPRESSIONS~.
...
the Oracle Grid Engine or PBS/Torque.
...
#+begin_src lisp
  (define-program example-of-prologue
    ()
    (:prologue
...
functions which generate shell scripts which perform the necessary
invocations of the program to execute an entire tree.

** Sun / Oracle Grid Engine
The function ~PREPARE-OGE-SCRIPT~ is used to produce scripts which use
the Oracle Grid Engine to invoke a specified program.
#+begin_src lisp
  (defun prepare-oge-script (directory area tree program &key (if-exists :error) (output "stdout/") (error :output)))
#+end_src

~DIRECTORY~ is the location to store the generated files, ~AREA~ is a
prepared area, ~TREE~ is the tree to be executed and ~PROGRAM~ is the
pathname to the generated program or a list containing the path to a
~LISP-EXECUTABLE:EXECUTABLE~ component in an ASDF system.
...
- ~oge-program-arguments~ :: A user editable file for passing command
...
- ~:SUPERSEDE~ :: Overwrite ~oge.sh~, ~oge-program.sh~ and
                  ~oge-data.sexp~. Do not touch
                  ~oge-program-arguments~ and ~oge-arguments~.
...
The default contents for the files ~oge-program-arguments~ and
...
it prepares a script that uses ~xargs(1)~ as the scheduler.
#+begin_src lisp
  (defun prepare-xargs-script (directory area tree program &key (if-exists :error)))
#+end_src

~DIRECTORY~ is the location to store the generated files, ~AREA~ is a
prepared area, ~TREE~ is the tree of experiments to be executed and
~PROGRAM~ is the pathname to the generated program or a list
containing the path to a ~LISP-EXECUTABLE:EXECUTABLE~ component in an
ASDF system.

The files generated in ~DIRECTORY~ are as follows
- ~xargs.sh~ :: The script which invokes ~xargs(1)~ to run all of the
                experiments.
- ~xargs-program.sh~ :: A shell script which incorporates any
     arguments in ~xargs-program-arguments~ in to the invocation of
     ~PROGRAM~.
- ~xargs-program-arguments~ :: A user editable file for passing
     command line arguments to ~PROGRAM~.
- ~xargs-arguments~ :: A user editable file for passing command line
     arguments to ~xargs~.
- ~xargs-data.sexp~ :: A file containing the area and tree
     to be executed.

The keyword argument ~IF-EXISTS~ in ~PREPARE-XARGS-SCRIPT~ specifies
what to do if any of the above files exist. Valid values are
- ~:ERROR~ :: Signal an error if any of the above files exist.
- ~:SUPERSEDE~ :: Overwrite ~xargs.sh~, ~xargs-program.sh~ and
                  ~xargs-data.sexp~. Do not touch
                  ~xargs-program-arguments~ and ~xargs-arguments~.
- ~:SUPERSEDE-ALL~ :: Overwrite all of the above files.

The default contents for the files ~xargs-program-arguments~ and
~xargs-arguments~ are obtained from the dynamic variables
~*XARGS-PROGRAM-ARGUMENTS-STRING*~ and ~*XARGS-ARGUMENTS-STRING*~
respectively.

** PBS / Torque
The function ~PREPARE-PBS-SCRIPT~ is like ~PREPARE-OGE-SCRIPT~, but it
prepares a script that invokes the PBS command ~qsub~.
#+begin_src lisp
  (defun prepare-pbs-script (directory area tree program &key (if-exists :error) (output "stdout/") (error :output)))
#+end_src
...
The files generated in ~DIRECTORY~ are as follows
- ~pbs.sh~ :: The script which invokes ~qsub~ .
- ~pbs-program.sh~ :: A shell script which incorporates any
     arguments in ~pbs-program-arguments~ in to the invocation of
     ~PROGRAM~.
- ~pbs-program-arguments~ :: A user editable file for passing
     command line arguments to ~PROGRAM~.
- ~pbs-arguments~ :: A user editable file for passing command line
     arguments to ~qsub~.
- ~pbs-data.sexp~ :: A file containing the area and tree
     to be executed.
- ~pbs-script-*.sh~ :: An intermediary shell script which invokes
     ~pbs-program.sh~ for a specific depth of ~TREE~. These shell
     scripts are required due to limitations in Torque's
     implementation of PBS.

The keyword argument ~IF-EXISTS~ in ~PREPARE-PBS-SCRIPT~ specifies
what to do if any of the above files exist. Valid values are
- ~:ERROR~ :: Signal an error if any of the above files exist.
- ~:SUPERSEDE~ :: Overwrite ~pbs.sh~, ~pbs-program.sh~ and
                  ~pbs-data.sexp~. Do not touch
                  ~pbs-program-arguments~ and ~pbs-arguments~.
- ~:SUPERSEDE-ALL~ :: Overwrite all of the above files.

The default contents for the files ~pbs-program-arguments~ and
~pbs-arguments~ are obtained from the dynamic variables
~*PBS-PROGRAM-ARGUMENTS-STRING*~ and ~*PBS-ARGUMENTS-STRING*~
respectively.
