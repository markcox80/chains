...
#+begin_src lisp
  ...
  (defmethod perform ((object geometric-cross-validation) &key)  
    (synthesize-initial-guess (input-data) (distance-away-from-ground-truth)))
#+end_src

The functions ~INPUT-DATA~ and ~DISTANCE-AWAY-FROM-GROUND-TRUTH~ are
special functions that represent the input data to the
~GEOMETRIC-CROSS-VALIDATION~ block as depicted in the above diagram.

Implementing these functions is performed using the ~DEFINE-OUTPUT~
macro.
#+begin_src lisp
  (define-output input-data ((occlusion synthetic-occlusion))
    (block-value occlusion))
  
  (define-output distance-away-from-ground-truth ((object distance-away-from-ground-truth))
    (distance object))
#+end_src
